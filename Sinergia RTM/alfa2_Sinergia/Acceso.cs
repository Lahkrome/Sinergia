...
namespace alfa2_Sinergia
{
    public partial class Acceso : Form
    {
        ...
        void acceso()
        {
            try
            {
                if (tbUser.Text.Length < 16 || tbPass.Text.Length < 51)
                {
                    if (tbUser.Text.Length > 0 || tbPass.Text.Length > 0)
                    {
                        miconexion.Open();
                        SqlCommand consulta = new SqlCommand(" select * from tblUser where Usuario='" + tbUser.Text + "'and Pass='" + tbPass.Text + "'", miconexion);
                ...
                {
                    MessageBox.Show("Largo exedido, Consulte el manual de usuario",
                             "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Information);    
                }
